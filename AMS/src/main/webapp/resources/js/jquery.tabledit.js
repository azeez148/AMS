...
//                        $table.find('tr:gt(0)').append('<td style="white-space: nowrap; width: 1px;"><div class="tabledit-toolbox" style="margin-left: -5px; text-align: right;">' + editButton + saveButton + cancelButton + removeButton + '</div></td>');
                        $table.find('tr:gt(0)').append('<td style="white-space: nowrap; width: 1px;"><div class="tabledit-toolbox" style="margin-left: -5px; text-align: right;">' + editButton + saveButton  + removeButton + '</div></td>');
                    }
                }
            }
        };
        ...
        if (settings.editButton) {
            ...
            $table.on('click', 'button.tabledit-save-button', function(event) {
                if (event.handled !== true) {
                    ...
                    var rowIndex = $(this).closest('tr').index();//mah
//                    var lastIndex = $(this).find('tr: last').index(); 
                    var lastIndex = $('#example tr:last').index();
                    var name = $(this).parents('tr').find('#name').text();
                    var type = $(this).parents('tr').find('#type').text();
                    var percentage = $(this).parents('tr').find('#percentage').text();
                    
                    var active = $(this).parents('tr').find('#active').text().split(" ")[0];
                    
//                    $.ajax({
//            			type: "POST",
//            			url: "group.html",
////            			data: "name="+name+"&type="+type+"&percentage="+percentage+"&percentage="+percentage,,
...
//            		});
                    ...
                    $.ajax({
                    	  ...
                    	  data: {id: id, name: name, type: type, percentage: percentage, active: active },
                    	  success: function(response){
              					
              					var message = response;
              					if(message.indexOf("Success") > -1)
              						{
              						$("#successGroup").html(message);
              						$("#successGroup").show();
              						$("#failGroup").hide();
              						$("#successGroup").html(message);
              						$("#btn"+lastIndex).click();
              						if(message.indexOf("Successfully Saved") > -1)
              							location.replace("showLastPage")
//              						else{
